...
## 最近 3 次重大改动摘要
- **[066] 2026-06-01 v0.6.2**：mobile_app（iOS/Android 客服 App）同步 [065] 的「已联系」过滤能力。改动：① `mobile_app/lib/api/models.dart` Conversation 类加 `bool hasVisitorMsg` 字段 + fromJson 解析 `has_visitor_msg` + 新增 toJson/copyWith + 新增 `isContacted` getter（兜底 unread>0）；② `mobile_app/lib/state/app_state.dart` 加 `ValueNotifier<String> filterMode` + `filteredConvs/contactedCount` getter + `setFilterMode` 方法，WSS `_onEnvelope` type=chat 在 fromVisitor 分支追加 `c.hasVisitorMsg=true`，新增 voice 通话 sys 事件（kind='voice'）也置 true 但不累计 unread；③ `mobile_app/lib/pages/conversations_page.dart` AppBar.bottom 加 Material 3 原生 `SegmentedButton<String>`（零自定义样式）展示「全部 (N) / 已联系 (M)」实时计数，列表数据源切换为 filteredConvs，空态文案随 mode 切换。**unread 累计逻辑不动**——mobile_app 本来就只在 fromVisitor 才 ++，无 [065] 那个 sys 算未读的 bug。
